...
def compound_tversky_cce(weights, ndim, alpha=0.7, gamma=0):
    """sum of weighted cce and dice loss"""
    _cce = weighted_categorical_crossentropy(weights, ndim, gamma)
    _tversky = weighted_tversky_loss(weights, ndim, alpha, 1 + 0.2 * gamma)
...
class StarDistBase(BaseModel):
    ...
    def prepare_for_training(self, optimizer=None):
        ...
        if self._is_multiclass():
            ...
            prob_class_loss = compound_tversky_cce(
                self.config.train_class_weights,
                ndim=self.config.n_dim,
                gamma=self.config.train_focal_gamma,
            )
