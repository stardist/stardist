...
def generic_masked_loss(mask, loss, weights=1, receptive_field = None, norm_by_mask=True, reg_weight=0, reg_penalty=K.abs):
    
    def _loss(y_true, y_pred):
        ss = (slice(None),)
        if receptive_field is not None:
            ss = ss+tuple(slice(s1,-s2) for s1,s2 in receptive_field)

        actual_loss = K.mean(mask[ss] * weights * loss(y_true[ss], y_pred[ss]), axis=-1)
        norm_mask = (K.mean(mask[ss]) + K.epsilon()) if norm_by_mask else 1
            
        if reg_weight > 0:
            reg_loss = K.mean((1-mask[ss]) * reg_penalty(y_pred[ss]), axis=-1)
            return actual_loss / norm_mask + reg_weight * reg_loss
        else:
            return actual_loss / norm_mask
    return _loss

def masked_loss(mask, penalty, reg_weight, receptive_field = None, norm_by_mask = True):
    loss = lambda y_true, y_pred: penalty(y_true - y_pred)
    return generic_masked_loss(mask, loss, reg_weight=reg_weight,
...
def masked_loss_mae(mask, reg_weight=0, receptive_field = None, norm_by_mask=True):
    return masked_loss(mask, K.abs, reg_weight=reg_weight,
                       receptive_field = receptive_field,
                       norm_by_mask=norm_by_mask)

def masked_loss_mse(mask, reg_weight=0, receptive_field = None, norm_by_mask=True):
    return masked_loss(mask, K.square, reg_weight=reg_weight,
                       receptive_field = receptive_field,
                       norm_by_mask=norm_by_mask)
...
def valid_binary_crossentropy(receptive_field = None):
    def binary_crossentropy(y_true, y_pred):
        ss = (slice(None),)
        if receptive_field is not None:
            ss = ss+tuple(slice(s1,-s2) for s1,s2 in receptive_field)
        return K.mean(K.binary_crossentropy(y_true[ss],
                                            y_pred[ss]), axis = -1)
...
class StarDistBase(BaseModel):
    ...
    def prepare_for_training(self, optimizer=None):
        ...
        if self.config.use_valid_mask:
            _field = tuple((s[0]//g,s[1]//g) for s,g in zip(self._compute_receptive_field(), self.config.grid))
        else:
            _field = None

        print("valid mask borders (output):", _field)
        dist_loss = {'mse': masked_loss_mse, 'mae': masked_loss_mae}[self.config.train_dist_loss](input_mask, reg_weight=self.config.train_background_reg, receptive_field = _field)
        # prob_loss = 'binary_crossentropy'
        prob_loss = valid_binary_crossentropy(receptive_field = _field)
