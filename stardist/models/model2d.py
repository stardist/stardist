...
Input, Conv2D, MaxPooling2D, Concatenate, UpSampling2D = keras_import(
    "layers", "Input", "Conv2D", "MaxPooling2D", "Concatenate", "UpSampling2D"
)
...
class StarDistData2D(StarDistDataBase):
    ...
    def __getitem__(self, i):
        ...
        if self.shape_completion:
            ...
            dist_mask = np.stack(
                [edt_prob(lbl[self.b][self.ss_grid[1:3]]) for lbl in Y_cleared]
            )
        else:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not len(self.train_loss_weights) == (2 if self.n_classes is None else 3):
            raise ValueError(
                f"train_loss_weights {self.train_loss_weights} not compatible with n_classes ({self.n_classes}): must be 3 weights if n_classes is not None, otherwise 2"
            )
        ...
        if not len(self.train_class_weights) == (
            ...
            raise ValueError(
                f"train_class_weights {self.train_class_weights} not compatible with n_classes ({self.n_classes}): must be 'n_classes + 1' weights if n_classes is not None, otherwise 2"
            )
...
class StarDist2D(StarDistBase):
    ...
    def _build(self):
        ...
        if self.config.backbone == "unet":
            ...
            unet_base = unet_block_v2(
                n_depth=unet_kwargs["n_depth"],
                n_filter_base=unet_kwargs["n_filter_base"],
                kernel_size=(3, 3),
                strides=unet_kwargs["pool"],
                block="conv_bottleneck",
            ...
            unet_base = tuple(
                UpSampling2D(
                    tuple(p**i for p in unet_kwargs["pool"]), interpolation="bilinear"
                )(x)
                for i, x in enumerate(unet_base)
            )
            ...
            unet_base = fpn_block(
                head_filters=unet_kwargs["n_filter_base"], **unet_kwargs
            )(pooled_img)
        else:
        ...
        )(_head(unet, self.config.unet_n_filter_base // 2))

        output_dist = Conv2D(
            self.config.n_rays, (1, 1), name="dist", padding="same", activation="linear"
        )(_head(unet, self.config.n_rays))
        ...
        if self._is_multiclass():
            ...
            output_prob_class = Conv2D(
                ...
                padding="same",
                activation="softmax",
            )(_head(unet_class, self.config.unet_n_filter_base // 2))
        ...
        if self.config.train_tensorboard:
            ...
            if self._is_multiclass():
                ...
                output_slices[2][1 + channel] = slice(
                    1,
                    1 + ((self.config.n_classes + 1) // _n) * _n,
        ...
        **nms_kwargs,
        ...
        if scale is not None:
            ...
            if not (isinstance(scale, dict) and "X" in scale and "Y" in scale):
                raise ValueError(
                    "scale must be a dictionary with entries for 'X' and 'Y'"
                )
    ...
    def _axes_div_by(self, query_axes):
        self.config.backbone in ("unet", "unetplus", "mrunet", "fpn") or _raise(
            NotImplementedError()
        )
