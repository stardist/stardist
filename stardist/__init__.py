...
def _py_deprecation(ver_python=(3,6), ver_stardist='0.9.0'):
     ...
     if sys.version_info[:2] == ver_python and LooseVersion(__version__) < LooseVersion(ver_stardist):
         print(f"You are using Python {'.'.join(map(str,ver_python))}, which will no longer be supported in StarDist {ver_stardist}.\n"
               f"→ Please upgrade to Python {'.'.join((str(ver_python[0]),str(ver_python[1]+1)))} or later.", file=sys.stderr, flush=True)
_py_deprecation()
del _py_deprecation